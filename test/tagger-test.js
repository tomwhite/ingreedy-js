...
describe('tokenize', function() {
    it('should parse grams', function() {
        assert.deepEqual(tagger.tokenize("100 grams flour"), ["100", "grams", "flour"]);
    });
    ...
    it('should parse tsp', function() {
        assert.deepEqual(tagger.tokenize("2 tsp sugar"), ["2", "teaspoon", "sugar"]);
    });
    it('should parse tsp with period', function() {
        assert.deepEqual(tagger.tokenize("2 tsp. sugar"), ["2", "teaspoon", "sugar"]);
    });
    it('should parse complex line', function() {
        assert.deepEqual(tagger.tokenize("100 grams big, green apples (not rotten)"), ["100", "grams", "big", ",", "green", "apples", "(", "not", "rotten", ")"]);
    });
});
...
describe('getFeatures', function() {
    it('should parse units', function() {
        ...
        assert.deepEqual(tagger.getFeatures("grams", 2, tokens), ["I2", "L4", "NoCAP", "NoPAREN"]);
    });
});
...
describe('insideParenthesis', function() {
    const tokens = ["100", "grams", "big", ",", "green", "apples", "(", "not", "rotten", ")"]
...
describe('smartJoin', function() {
    it('should return singular form for ingredient units', function() {
        assert.equal(tagger.smartJoin(["big", ",", "green", "apples", "(", "not", "rotten", ")"]), "big, green apples (not rotten)");
    });
});
...
describe('import_data', function() {
    ...
    it('should return display', function() {
        assert.equal(output[0]['display'], "<span class='qty'>1/2</span><span class='unit'>teaspoon</span><span class='comment'>fresh</span><span class='name'>thyme leaves</span><span class='comment'>, finely chopped</span>");
    }); 
    it('should return input', function() {
        assert.equal(output[0]['input'], "1/2 teaspoon fresh thyme leaves, finely chopped");
    }); 
    ...
    it('should return display', function() {
        assert.equal(output[1]['display'], "<span class='name'>Black pepper</span><span class='other'>,</span><span class='comment'>to taste</span>");
    });
    it('should return input', function() {
...
describe('export_data', function() {
    it('should convert ingredient lines', function() {
        ...
        assert.equal(tagger.export_data(lines), `100	I1	L4	NoCAP	NoPAREN
