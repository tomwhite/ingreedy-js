...
CRFNode().then(function (Module) {
  ...
  function printFoodsNotInFoodMap(inputFile) {
    ...
    const foodsWithCarbs = foods.map((food) =>
      measures.calculateCarbsInFood(food, fallbackToSearch)
    );
    const outcomes = {};
    for (const foodWithCarbs of foodsWithCarbs) {
      outcomes[foodWithCarbs.outcome] =
        (outcomes[foodWithCarbs.outcome] || 0) + 1;
      if (foodWithCarbs.outcome === Symbol.for(outcome)) {
        console.log(
          `${inputFile}|${foodWithCarbs.reasonText}|${foodWithCarbs.input}|${
            foodWithCarbs.qty || ""
          }|${foodWithCarbs.unit || ""}|${
            foodWithCarbs.food ? foodWithCarbs.food.name || "" : ""
          }`
        );
      }
    }
